---
title: Avatar
description: Representation of entity. Can render an ENS avatar if Wagmi is setup in your app.
---

```tsx
import { Avatar } from 'degen'
```

```tsx live=true expand=true
<Avatar ens={false} label="Noun 97" src={avatars.noun97} />
```

## Props

<PropsTable sourceLink={sourceLink} types={types} />

## Sizes

```tsx live=true
<Stack align="center" direction="horizontal" wrap>
  <Avatar ens={false} label="Noun 97" size="24" src={avatars.noun97} />
  <Avatar ens={false} label="Noun 97" size="16" src={avatars.noun97} />
  <Avatar ens={false} label="Noun 97" size="12" src={avatars.noun97} />
  <Avatar ens={false} label="Noun 97" size="10" src={avatars.noun97} />
</Stack>
```

## Placeholder

A placeholder can be shown by setting the `placeholder` prop to true. The color is determined by the (optional) `address` prompt. It will default to blue. The placeholder will also be shown if the `src` is invalid and can not be loaded.

```tsx live=true expand=true
<Stack align="center" direction="horizontal" wrap>
  <Avatar
    address="0x62500C50fh999506597609B640F1D3672A0082Dc"
    ens={false}
    placeholder
  />
  <Avatar
    address="0x78Bddfs96cC5c0d6B506597609B64F1D999D82Dc"
    ens={false}
    placeholder
  />
  <Avatar
    address="0x625BBa96Cc5c0d6B506597609B64F1D3672D82Dc"
    ens={false}
    placeholder
  />
  <Avatar ens={false} src="bad/url" />
</Stack>
```

## Shapes

```tsx live=true
<Stack align="center" direction="horizontal" wrap>
  <Avatar
    ens={false}
    label="Noun 97 circle"
    shape="circle"
    size="24"
    src={avatars.noun97}
  />
  <Avatar
    ens={false}
    label="Noun 97 square"
    shape="square"
    size="24"
    src={avatars.noun97}
  />
</Stack>
```

## Border

By default, there is a light border around the edge to define image edges on similarly colored backgrounds. This can be disabled by using the `noBorder` prop.

```tsx live=true
<Stack direction="horizontal">
  <Avatar
    ens={false}
    size="24"
    src={
      mode === 'light'
        ? 'https://images.mirror-media.xyz/publication-images/152cd86f-45ad-4f48-8971-4dd33fde5bc1.png'
        : 'https://images.mirror-media.xyz/publication-images/5eaba6b2-b9ee-4bea-aeac-75796d53798f.jpg'
    }
  />
  <Avatar
    ens={false}
    noBorder
    size="24"
    src={
      mode === 'light'
        ? 'https://images.mirror-media.xyz/publication-images/152cd86f-45ad-4f48-8971-4dd33fde5bc1.png'
        : 'https://images.mirror-media.xyz/publication-images/5eaba6b2-b9ee-4bea-aeac-75796d53798f.jpg'
    }
  />
</Stack>
```

## next/image

If you are using Next.js and configured [`next/image`](https://nextjs.org/docs/api-reference/next/image) appropriately, you can pass it in with the `as` prop.

```tsx live=true expand=true
<Avatar as={NextImage} ens={false} label="Noun 97" src={avatars.noun97} />
```
